...
function Enduserregistersignup() {
  ...
  const userregister = (f_name, l_name, email) => {
    ...
    axios
      .post(globeVar + "user/ragistration/sed_email", item)
      .then((response) => {
        if (response.data.success === 1) {
          // set the state of the user
          Swal.fire({
            position: "top-end",
            title:
              '<span style="font-size:1.4rem";><small>Email Send successfully</small></span>',
        ...
        <div class="container section-padding">
          <div class="row justify-content-center flex-column-reverse flex-lg-row">
                      ...
                      <div class="form-row align-items-baseline">
                        ...
                        <div class="form-group col-sm pr-4">
                          <input
                            type="submit"
                            value="Signup"
                            class="btn gradient-btn px-5"
                          />
                        </div>
                      </div>

                      <div>
                        <p class="weight-600 mb-0 ">
                          Already have an account?{" "}
                          <Link to={`/login`} class="color-txt">
                            Login
                          </Link>{" "}
                        </p>
                      </div>
            ...
            <div class="col-12 col-md-12 col-lg-10 col-xl-4 text-center">
              <img
                src={formsideimg}
                class="radius-shadow-img img-fluid mb-4"
                alt=""
              />
            </div>
          </div>
