...
const Footer = () => {
  ...
  //api of footer text
    ...
    //api call of socialmedia icons and links
       const socialmedia = () => {
      ...
      <div class="row">
        <div class="col-12 col-sm-12 col-lg-4 ">
           ...
           return <div class="row" key={mon.id}>
             ...
             <div class="col col-lg-12"><img src={"http://localhost:8000/images/"+mon.content} alt="" class="img-fluid mb-3" /></div> </div>}})}
            <div class="social-links col-auto col-lg-12 mb-3">
               {socialmedias.map(con =>{
              return  <a href={con.link} target="_blank" rel="noopener noreferrer" key={con.id}><img src={"http://localhost:8000/images/"+con.logo} alt="" /></a>
              })}  
            </div>
            
            {setting.map(mons=>{
              if(mons.name == "footer_text"){ 
           return <div class="row" key={mons.id}> <p class="col-12 txt my-2 color-txt pr-lg-5 d-none d-md-block">{mons.content}</p></div>}})}
           {setting.map(monss=>{
              if(monss.name == "copyright_text"){ 
           return <div class="row" key={monss.id}> <p class="txt my-2 col-12 d-none d-lg-block">{monss.content}</p></div>}})}
          </div>
